...
\eje [3.12] \textit{Este es difícil, si sale ya tienen cocinado Cinemática.} Un proyectil es disparado hacia arriba por un plano inclinado de ángulo $\phi$ con una velocidad inicial $v_0$ a un ángulo $\theta_0$ con respecto a la horizontal del suelo (siendo $\theta_0 > \phi$ ).
\begin{figure}[H]
\begin{center}
\includegraphics[clip,width = .57\columnwidth]{p1_f13}
\end{center}
\end{figure}
a) Muestre que el proyectil viaja una distancia d = [2 $v_0 ^2$ cos($\theta_0$) sin( $\theta_0$ - $\phi$ )]/[g cos$^2$($\phi$ )] por el plano inclinado.

¿Para qué valor de $\theta_0$ es “d” un máximo, y cuál sería ese valor?
